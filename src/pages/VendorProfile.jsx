...
export default function VendorProfile() {
  ...
  if (!vendor) {
    return (
      <>
        <SEOHead title="Vendor Not Found" path={`/vendors/${slug}`} />
        <section className="min-h-screen bg-ivory flex items-center justify-center px-6 pt-24">
          <div className="text-center">
            <p className="text-5xl mb-4" aria-hidden="true">🔍</p>
            <h1 className="text-3xl font-serif text-plum mb-4">Vendor not found</h1>
            <p className="text-plum/50 mb-8">This vendor profile doesn't exist or may have moved.</p>
            <Link to="/vendors" className="bg-rose text-white px-8 py-3 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-plum transition">
              Back to All Vendors
            </Link>
          </div>
        </section>
  ...
  const { name, category, location, from, rating, badge, img, longBio, tags, phone, founded, social, services, gallery } = vendor
  const waMsg = encodeURIComponent(`Hi! I found ${name} on Harusi Planners and would love to enquire about availability and pricing.`)
  ...
  return (
    <>
      <SEOHead
        ...
        description={`${name} is a ${category.toLowerCase()} based in ${location}. ${vendor.bio} Starting from ${from}.`}
        image={img}
        path={`/vendors/${slug}`}
      />
      ...
      <section className="relative h-[55vh] min-h-[360px] overflow-hidden">
        ...
        >
          <ArrowLeft size={14} aria-hidden="true" /> All Vendors
        </Link>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-10 py-16 pb-28 md:pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-14">
            ...
            {gallery?.length > 0 && (
              <section>
                <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-rose/60 mb-3">Portfolio</p>
                <h2 className="text-3xl font-serif text-plum mb-6">Gallery</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {gallery.map((src, i) => (
                    <button key={i} onClick={() => setLightboxIndex(i)}
                      ...
                      <img src={src} alt={`${name} portfolio ${i + 1}`}
            ...
            {services?.length > 0 && (
              <section>
                <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-rose/60 mb-3">Services & Pricing</p>
                <h2 className="text-3xl font-serif text-plum mb-6">Packages</h2>
                ...
                <p className="text-xs text-plum/40 mt-4">All prices are starting rates. Final quotes depend on date, location, and scope.</p>
              </section>
            )}
          </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="bg-plum rounded-3xl p-7 text-white sticky top-24">
              ...
              <p className="text-white/30 text-xs mb-7">Excl. VAT · Final quote on enquiry</p>
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
            ...
            <div className="bg-ivory rounded-3xl p-7 space-y-4">
              <h3 className="text-xs font-bold uppercase tracking-widest text-plum/40 mb-2">Details</h3>
            ...
            <div className="bg-white rounded-3xl p-7">
              ...
              <div className="space-y-4">
                {VENDORS.filter(v => v.category === category && v.slug !== slug).slice(0, 2).map(v => (
                  <Link key={v.slug} to={`/vendors/${v.slug}`} className="flex items-center gap-3 group">
              ...
              <Link to="/vendors" className="block text-center text-[10px] font-bold uppercase tracking-widest text-rose mt-6 hover:text-plum transition">
                See All Vendors ⟶
              </Link>
            </div>
          </aside>
